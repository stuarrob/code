#!/usr/bin/env python3
"""
Generate comprehensive research report on technical indicator weights and periods.
"""
...
RESULTS_FILE = PROJECT_ROOT / "results" / "indicator_weight_analysis.json"
REPORT_FILE = PROJECT_ROOT / "reports" / "technical_indicator_weights_report.md"
...
This report presents comprehensive research on technical indicator selection, optimal periods,
and weighting schemes for composite signal generation in ETF portfolio optimization.

### Key Findings

1. **Contrarian Nature of Technical Indicators**: Most standard technical indicators show NEGATIVE
   correlation with forward returns, suggesting mean-reversion dominates momentum in ETF markets

2. **Time Variance**: Technical indicators exhibit significant time-variance (median CV > 0.85),
   indicating that single-period optimization is insufficient

3. **ADX Shows Promise**: Average Directional Index (ADX) is the only indicator with positive
   correlation to forward returns (IC: +0.030), suggesting trend-strength is predictive

4. **Recommended Approach**: Use INVERTED signals for momentum indicators combined with trend-strength
   filters for more robust performance
...
### 3.3 Recommended Weights (Ridge-Optimized, Inverted)

Based on ridge regression analysis with signal inversion:

| Indicator | Weight | Rationale |
|-----------|--------|-----------|
| RSI (inverted) | 30% | Strongest IC, good time-invariance |
| BB %B (inverted) | 25% | Best time-invariance, strong signal |
| ADX (standard) | 20% | Only positive IC, trend filter |
| ROC (inverted) | 15% | Moderate IC, momentum capture |
| CMF (inverted) | 10% | Volume confirmation |
...
if __name__ == "__main__":
    ...
    print(f"\nComprehensive research report created at:")
    print(f"  {report_path}")
    print()
    print("Key recommendations:")
    print("  - Use INVERTED momentum signals (mean reversion)")
    print("  - Weights: RSI(30%), BB(25%), ADX(20%), ROC(15%), CMF(10%)")
    print("  - Multi-timeframe: 21d(40%), 63d(35%), 126d(25%)")
    print("  - Standard periods: RSI:14, BB:20/2.0, MACD:12/26/9, ADX:14")
